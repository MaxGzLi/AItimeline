...
- **周边概念少于 3 个的卡不画图**（碰撞剔除之后仍不足 3 个也一样，`degenerate: true` 交给前端保持纯文字）：一两个周边节点画出来就是「一条横线加一个点」，那两三个标签和卡片正下方的概念标签一字不差，零信息增量，却要吃掉卡片约 30% 的高度。真实快照 105 张非播报卡实测：门槛取 ≥3 时 85 张（81%）仍能画图、20 张退为纯文字；取 ≥2 只挡掉 5 张，挡不住这个毛病。
