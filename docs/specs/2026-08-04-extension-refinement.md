...
> **2026-08-04 真机验收改版**:原方案是「按钮变『点开存全文』导流到详情页」。
> 真机实测推翻:① X 的折叠标记是 `<button>显示更多`(没有 href),点击**就地
> 展开**正文、不跳转(154→616 字,约 1 秒);② 中文时间线上折叠比例很高
> (实测 6 条里 4 条),按原方案大多数推文都要跳走一趟,把「顺手一键存」的
> 核心心流毁了。改为:按钮照常显示「保存」,点击时先就地展开再存全文,一次
> 点击、不离开时间线;2.5 秒内没展开成功才退回详情页兜底。另实测发现广告位
> 有正文有操作栏但无永久链接(存不了),现在不给它挂按钮。
...
- [x] P0-4:折叠推文点「保存」先就地展开再存,存进去的 capturedText 是完整
      正文;引用块内的折叠标记不触发展开。真机实测记录见
      docs/e2e/runs/2026-08-04-extension-refinement/notes.md。
      (X Article 时间线预览卡的拦截未做,列入 README 已知限制。)
